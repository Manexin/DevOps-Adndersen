...
if [ ! -e ./quotes.json ]
	then #download actual JSON
	curl -so ./quotes.json https://yandex.ru/news/quotes/graph_2000.json
fi
...
res_year=$(echo "$vol" | sort -t':' -nk2 | head -n1 | cut -d':' -f1)
res_vol=$(echo "$vol" | sort -t':' -nk2 | head -n1 | cut -d':' -f2)
echo -e " $res_year, volatility was $res_vol"
